...
#0.5 to 1,not done
    ...
    def __call__(self, net, dl, n_classes):
        ...
        #self.lb_map = {el['trainId']: el['color'] for el in labels_info}#color

        #diter = enumerate(lb_map),copy,preimg,kuo3color,replace,type to color
        #print ,verify
        ...
        for i, (imgs, label) in diter:
            ...
            #preds,
            #print(preds) [1,1024,2048]
            #print(list(preds.size()))

            #imgPre=Image.fromarray(np.reshape(preds, (1024, 2048))).convert('RGB')
            #save_image(imgPre, '.\\imgpre\\'+str(i) + '.png')
            #imgPre.show()
            ...
            # np.place(image_mask_copy,image_mask_copy!=15,[0]) # REMOVE ALL EXCEPT PEOPLE, == 0;black
            # np.place(image_mask_copy,image_mask_copy==15,[255]) # MAKE PEOPLE == 255;white
            ...
            imgFileName = osp.join(preImgDir, str(i) + '.png')
            #print('imgFileName',imgFileName)
            predsimg = preds.squeeze(0).cpu().numpy()
            cv.imwrite(imgFileName, predsimg)
            #img = Image.open('.\\imgpre\\' + str(i) + '.png')
            #img.show()
            #imgPre=Image.fromarray(np.reshape(preds, (1024, 2048))).convert('RGB')
            # save_image(imgPre, '.\\imgpre\\'+str(i) + '.png')
            # imgPre.show()
        ...
        if dist.is_initialized():
            dist.all_reduce(hist, dist.ReduceOp.SUM)
        ious = hist.diag() / (hist.sum(dim=0) + hist.sum(dim=1) - hist.diag())
        print('ious',ious)##print each ious
        miou = ious.mean()
        #三个小目标单项的iou
        print('miou', miou)
